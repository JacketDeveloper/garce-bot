...
Fg.send2ButtonLoc(from, welm, capt, 'Sígueme en Instagram\nhttps://www.instagram.com/jacket_nakche', '⦙☰ MENU', '/menu', '⏍ INFO GP', '/infogp', false, {
	      contextInfo: { 
            mentionedJid: Fg.parseMention(capt)
	      } 
	    }); //--
...
if(!isCmd && isChatbot === true){
 ...
 // if(!isPremium) return
  ...
  m.reply(result.success.replace('simsimi', 'Garce').replace('Simsimi', 'Garce').replace('sim simi', 'Garce'))
}
